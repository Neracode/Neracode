    ...
    <MyContainer containerId="mentors" background="mt-20">
      <Splide
        ...
        <div className="custom-wrapper">
          <div className="grid gap-4 grid-cols-[auto_auto] max-md:grid-cols-1 items-end">
            <div className="grid gap-2">
              ...
              <div className="text-lg mt-3">
                ...
                akan menemukan komunitas mentor yang luar biasa, yang
                terdiri dari para profesional dan pakar di berbagai
                bidang. Para mentor kami memiliki dedikasi untuk
                membantu Anda dalam perjalanan Anda, baik itu dalam
                mengembangkan karir, mempelajari keterampilan baru,
                atau mencapai tujuan pribadi.
              </div>
            </div>
          ...
          <SplideTrack>
            {MentorsData.map(
              ({ name, posisi, angkatan, img }, index) => (
                <SplideSlide
                  key={index}
                  className={`h-full duration-700 ease-[cubic-bezier(.75, .2, .2, 0.94)] transition-all delay-250 ${
                    isArrowClicked
                      ? '[&:nth-child(2n)]:mt-[-1.5rem]'
                      : '[&:nth-child(2n+1)]:mt-[-1.5rem]'
                  }`}>
                  <Card
                    posisi={posisi}
                    nama={name}
