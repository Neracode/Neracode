...
function FeedBackConfig() {
  ...
  useEffect(() => {
    const fetchConfig = async () => {
      try {
        ...
        setMessage('Gagal mengambil konfigurasi: ' + error.message);
      }
    };
    fetchConfig();
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!token) {
      setMessage('Admin token diperlukan');
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          data.error || 'Gagal memperbarui konfigurasi'
        );
      }
              ...
              <div>
                ...
                <div className="space-y-2">
                  <input
                    ...
                    onChange={(e) =>
                      handleHourChange('start', e.target.value)
                    }
                    className="block p-3 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 shadow-sm focus:ring-primary-500 focus:border-primary-500"
                    required
                    step="3600"
              ...
              <div>
                ...
                <div className="space-y-2">
                  <input
                    ...
                    onChange={(e) =>
                      handleHourChange('end', e.target.value)
                    }
                    className="block p-3 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 shadow-sm focus:ring-primary-500 focus:border-primary-500"
                    required
                    step="3600"
